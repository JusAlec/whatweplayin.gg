...
2. **Create KV namespace**:
   ```
   pnpm --filter @gno/worker exec wrangler kv:namespace create KV
   ```
...
4. **Write the secret to KV**:
   ```
   pnpm --filter @gno/worker exec wrangler kv:key put --binding=KV "group:mygroup:secret" "<paste secret>"
   ```
5. **Deploy the worker**:
   ```
   pnpm --filter @gno/worker deploy
   ```
   Note the worker URL (e.g. `https://gamenight-os.<account>.workers.dev`).
6. **Connect repo to Cloudflare Pages**:
   - Cloudflare dashboard → Pages → Connect to Git → select repo
   - Build command: `pnpm install && pnpm --filter @gno/site build`
