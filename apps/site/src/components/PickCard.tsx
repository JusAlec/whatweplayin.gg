...
export default function PickCard({ pick, rank, expanded = false, onLockIn }: Props) {
  ...
  return (
    <article
      className={`bg-panel border border-border rounded p-4 ${rank === 1 ? '' : 'opacity-90'}`}
    >
      <header className="flex items-start justify-between mb-2">
        <div>
          <h2 className={rank === 1 ? 'text-2xl font-bold' : 'text-lg font-semibold'}>
            {game.name}
          </h2>
