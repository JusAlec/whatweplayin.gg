...
describe('generateMagicLinkToken', () => {
  test('inserts a row with 15-minute expiry and returns the token', async () => {
    ...
    const row = await env.DB.prepare('SELECT * FROM magic_link_tokens WHERE token = ?').bind(token).first();
...
describe('validateMagicLinkToken', () => {
  ...
  test('marks token as used after validation', async () => {
    ...
    const row = await env.DB.prepare('SELECT used_at FROM magic_link_tokens WHERE token = ?').bind(token).first();
